...
function acceptMessage(message, callback) {
    goToMessages(function() {
        var messageType = page.evaluate(function(message) {
            var messageNodes = document.querySelectorAll('.feedmessage > p');
            for (var i = 0; i < messageNodes.length; i++) {
                if (messageNodes[i].textContent.replace(/\n/g, " ").indexOf(message) > -1) {
                    ...
                    if (btn) {
                        ...
                        //It's one of the new social action buttons with a 'Respond' button
                        btn = messageNodes[i].parentNode.querySelector('.social-event');
                        btn.click();
                        return 'social';
                    }
                }
        ...
        if (messageType == 'simple') {
            loge("Social action accepted: " + message);
            setTimeout(callback, 500);
        } else if (messageType == 'social') {
            waitForReady(function() {
                waitForAjax(function() {
                    var storylets = getStorylets();
                    clickStorylet(storylets[0], function() {
                        chooseDefaultPlayer(function() {
                            loge("Social action accepted: " + message);
                            callback();
                        });
                    });
            ...
            loge("Error in accepting message.");
            loge(1/0);
        }
    });
}
...
//Do not call directly, use acceptNMessages
function acceptNMessagesR(numToAccept, messages, callback) {
    if (numToAccept > 0 && messages.length > 0) {
        acceptMessage(messages.pop(), function() {
            acceptNMessagesR(numToAccept - 1, messages, callback);
        })
    } else {
        callback();
    }
...
function acceptNMessages(numToAccept, callback) {
    getMessagesToYou(function(messages) {
        acceptNMessagesR(numToAccept, messages, callback);
    });
}
...
function getQualities(callback) {
    var openAllQualityCategories = function(callback) {
        page.evaluate(function() {
            var categories = document.querySelectorAll('.qualityCategory > a');
        ...
        var check = function(callback) {
            setTimeout(function() {
                var ready = page.evaluate(function() {
                    var ready = true;
                    var sections = document.querySelectorAll('.qualitiesToggleDiv');
                    for (var i = 0; i < sections.length; i++) {
                        if (sections[i].querySelector('.qualities') == null) {
                            var category = sections[i].previousElementSibling.querySelector('a');
                            category.click();
                            category.click();
                            ready = false;
                        }
    ...
    goToMyself(function() {
        openAllQualityCategories(function() {
            var qualitiesRaw = page.evaluate(function() {
                var qualityNodes = document.querySelectorAll('.qualities_rhs > p > strong');
                var qualities = [];
                
                for (var i = 0; i < qualityNodes.length; i++) {
                    qualities.push(qualityNodes[i].textContent.trim());
                }
            ...
            var qualities = {};
            for (var i = 0; i < qualitiesRaw.length; i++) {
                var words = qualitiesRaw[i].split(" ");
                var quality = "";
                var maybeNum;

                for (var j = 0; j < words.length; j++) {
                    maybeNum = parseInt(words[j]);
                    if (isNaN(maybeNum)) {
                        quality += words[j] + " ";
                    } else {
                        break;
                ...
                if (maybeNum) {
                    qualities[quality.trim()] = maybeNum;
                } else {
                    qualities[quality.trim()] = "n/a";
                }
            }
